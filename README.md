...
- `POST /api/obd/detect-ecu` - Detect ECU information (legacy)

### ECU Detection Routes (New)

- `POST /api/detect` - Detect ECU with full diagnostic information
  - Request body: `{ useCache?: boolean, userProtocol?: string, useMock?: boolean, mockData?: object }`
  - Returns: Protocol, VIN, ECU identifiers, supported modes/PIDs, manufacturer, timestamps
  - Features: Adapter initialization (ATZ, ATE0, ATL0, ATS0), auto protocol detection (ATSP0/ATDP), Mode 09 queries, Mode 22/UDS fallback, VIN-to-manufacturer mapping, PID file loading

- `GET /api/detect/cached` - Get previously cached detection result
  - Returns: Cached detection data or 404 if no cache exists

- `POST /api/detect/clear-cache` - Clear the detection cache
